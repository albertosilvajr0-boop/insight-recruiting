...
export async function sendApplicationReceipt(candidate, baseUrl) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 520px; margin: 0 auto;">
      <div style="background: #2563eb; color: white; padding: 20px 24px; border-radius: 12px 12px 0 0;">
        ...
        <p style="margin: 4px 0 0; opacity: 0.9; font-size: 13px;">San Antonio Dodge</p>
      </div>
      <div style="border: 1px solid #e5e7eb; border-top: none; padding: 24px; border-radius: 0 0 12px 12px; color: #374151;">
        ...
        <p style="margin: 0 0 16px; font-size: 14px; line-height: 1.55;">
          Thanks for applying for the <strong>${jobTitle || 'San Antonio Dodge'}</strong> position. We've received your resume and interview responses, and our hiring team will review them shortly — you'll hear back from us within 1 business day.
        </p>
        ${statusUrl ? `
  ...
  await sendEmail({
    to: email,
    subject: `We got your application — San Antonio Dodge`,
    html
  })
}
